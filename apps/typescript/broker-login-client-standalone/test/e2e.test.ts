...
test("standalone broker app logs in, calls tool, reads resource, then reuses cache", async (t) => {
...
test("standalone broker app resumes pending login and handles no resources", async (t) => {
...
test("standalone broker app refreshes expired cache and reports remote 401", async (t) => {
...
test("standalone broker app clears stale cached token rejected by MCP server", async (t) => {
  const fake = await startFakeServer();
  t.after(() => fake.close());
  const cacheRoot = tempCacheRoot();
